...
import com.javaboy.febs.auth.properties.FebsValidateCodeProperties;
import com.javaboy.febs.common.entity.FebsConstant;
import com.javaboy.febs.common.exception.ValidateCodeException;
import com.javaboy.febs.common.service.RedisService;
...
public class ValidateCodeService {
    ...
    /**
     * 生成验证码
     * create方法用于生成验证码。在前后端不分离的架构下，我们通过浏览器传输的jsessionid来和验证码图片一一对应，
     * 但前后的分离的模式下，客户端发送的请求并没有携带jsessionid（因为不再基于Session），
     * 所以我们需要客户端在发送获取验证码请求的时候，携带一个key（比如按一定算法生成的随机字符串，模拟jsessionid）
     * 来和验证码一一对应。于是我们在create里一开始就从请求中获取key值，然后根据验证码配置文件生成验证码，
     * 并将验证码字符保存到了Redis中（Redis Key为febs.captcha. + 客户端上送的key值，有效时间为配置文件定义的120秒 二分钟），
     * 并将验证码图片以流的形式返回给客户端
     *
    ...
    /**
     * createCaptcha方法通过验证码配置文件FebsValidateCodeProperties生成相应的验证码，
